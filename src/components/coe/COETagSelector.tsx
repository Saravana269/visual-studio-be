...
import { CreateTagDialog } from "../elements/CreateTagDialog";
import { useAuth } from "@/hooks/useAuth";

interface COETagSelectorProps {
  value: string | string[] | null;
  onChange: (value: any) => void;
}

export function COETagSelector({ value, onChange }: COETagSelectorProps) {
  ...
  const { data: availableTags = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        if (tagsError) {
          console.error("Error fetching COE tags:", tagsError);
          return [];
        }
        
        return tagsData || [];
      } catch (error) {
        console.error("Error in COE tag fetching:", error);
        return [];
      }
    },
    enabled: !!userId
