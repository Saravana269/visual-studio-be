...
export function ElementList({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {elements.map(element => <Card key={element.id} className="overflow-hidden flex flex-col">
          ...
          <CardHeader className="pb-2">
            ...
            {element.description && <p className="text-sm text-muted-foreground line-clamp-2">
                {element.description}
              </p>}
          </CardHeader>
          
          <CardContent className="pb-2 flex-1">
            {element.tags && element.tags.length > 0 && <div className="flex flex-wrap gap-2 mb-4">
                {element.tags.map(tag => <Badge key={tag} variant="secondary">{tag}</Badge>)}
              </div>}
