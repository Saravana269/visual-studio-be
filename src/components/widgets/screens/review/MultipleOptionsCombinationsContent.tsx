  ...
  return <div className="space-y-2 mt-4">
      ...
      <ScrollArea className="h-[300px]">
        <div className="space-y-2 pr-1">
          ...
          return <div key={index} className="flex items-center justify-between p-2 rounded border border-[#00FF00]/20 bg-black/30 hover:bg-[#00FF00]/10">
                  <span className="text-sm">{combination.join(", ")}</span>
                  <div className="flex items-center space-x-2">
                    {isConnected ? <ConnectionBadge connectionId={`combination_${index}`} onViewConnection={() => onViewConnection && onViewConnection(combination)} /> : onConnect}
                  </div>
                </div>;
