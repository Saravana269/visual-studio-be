...
export function WidgetGrid({
  ...
  if (isLoading) {
    return <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map(i => <Skeleton key={i} className="h-64 w-full rounded-md" />)}
      </div>;
  }
  if (widgets.length === 0) {
    return <div className="bg-muted p-8 rounded-md text-center">
        ...
        <Button onClick={onCreateClick} className="bg-[#9b87f5] hover:bg-[#7E69AB] text-white">
  ...
  return <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
      {widgets.map(widget => <Card key={widget.id} className="element-card relative flex flex-col overflow-hidden cursor-pointer" onClick={() => handleNavigateToScreens(widget)}>
          ...
          <div className="h-32 bg-muted flex items-center justify-center overflow-hidden">
            {widget.image_url ? <img src={widget.image_url} alt={widget.name} className="w-full h-full object-cover" /> : <div className="text-muted-foreground text-sm">No image</div>}
          </div>
          
          <div className="pb-2 p-3">
            <h3 className="font-semibold text-base">{widget.name}</h3>
            {widget.description && <p className="text-xs text-muted-foreground line-clamp-2">
                {widget.description}
              </p>}
          </div>
